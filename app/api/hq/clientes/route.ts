  ...
  try {
    ...
    if (id) {
      ...
      return NextResponse.json({
        ...
        proxima_acao: gerarProximaAcao(c),
      })
    }
    ...
    if (cs) query = query.eq('cs_responsavel', cs)
    if (scoreMin) query = query.gte('score_total', parseInt(scoreMin))
    if (scoreMax) query = query.lte('score_total', parseInt(scoreMax))
    ...
    return NextResponse.json({ clientes: result, total: result.length })
  } catch (err) {
